...
// all 32 registers in each index of the Snapshot array is initalized to 0
...
// registers are updated to match with what the instructions do to them
func updateRegisters(ins *Instruction, array *Snapshot) {
	if ins.op == "ADD" {
		//register rd's content is updated with the sum of register rd and rn's contents, which for now are only 0 cause
		//that's what they were initialized with
		array.regis[ins.rd].registerValue = array.regis[ins.rm].registerValue + array.regis[ins.rn].registerValue
	}
}
